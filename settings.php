...
if ($hassiteconfig) {
    ...
    $settings->add(
        new admin_setting_configselect(
            ...
            0,
            array_combine(store::$queueclasses, store::$queueclasses)
        )
    );
    $settings->add(
    ...
    if ($configuredqueue) {
        ...
        $warntext = $OUTPUT->notification(
            get_string(
                ...
                $configuredqueue->details()
            )." ".html_writer::link($url, "Details"),
            core\output\notification::NOTIFY_SUCCESS
        );
        $settings->add(
            new admin_setting_heading(
                'logstore_standardqueued/generalsettings',
                '',
                $warntext
            )
        );
        ...
        include __DIR__ . "/../standard/settings.php";
    }
}
